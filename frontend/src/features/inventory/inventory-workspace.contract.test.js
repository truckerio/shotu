...
test("stock has peer part and location modes with contextual physical counts", async () => {
  ...
  assert.match(locationStock, /<PositionCountPanel ref=\{countPanelRef\} key=\{`\$\{shopId\}:\$\{selectedPositionId\}`\} location=\{\{ id: shopId \}\} position=\{selected\}[\s\S]*canApplyInventoryCount=\{canApplyInventoryCount\}/);
  assert.match(locationStock, /selectedIsCountableLeaf \? <PositionCountPanel/);
  assert.match(locationStock, /aria-label=\{`More actions for \$\{locationName\(selected\)\}`\}/);
  assert.match(locationStock, /onAction=\{\(\) => countPanelRef\.current\?\.open\(\)\} textValue="Physical count"/);
  ...
  assert.match(countPanel, /isCurrent\(requestKey\)/);
  ...
  assert.match(countPanel, /On hand<\/small>/);
  assert.match(countPanel, /Variance<\/small>/);
  assert.match(countPanel, /aria-label=\{`Counted quantity for \$\{line\.partNumber\}`\}/);
  assert.match(countPanel, /observed \? `Reset counted quantity for \$\{line\.partNumber\} to on hand` : `Cancel counting \$\{line\.partNumber\}`/);
  assert.match(countPanel, /if \(observed\) saveLine\(line, line\.expectedQuantity\)/);
  assert.doesNotMatch(countPanel, /className="sr-only"/);
  assert.match(countPanel, /\/submit/);
  assert.match(countPanel, /Only these \{facts\.selectedCount\}/);
  assert.match(countPanel, /untouched \{facts\.untouchedCount === 1 \? "part stays" : "parts stay"\} unchanged/);
  assert.match(countPanel, /Submit for Admin review/);
  assert.match(countPanel, /Apply \$\{facts\.selectedCount\}/);
  assert.match(locationStock, /Exit count/);
  assert.match(locationStock, /Review count \(\{countMode\.selectedCount\}\)/);
  ...
  assert.match(countPanel, /useImperativeHandle\(actionRef/);
  assert.match(countPanel, /!expanded\) return null/);
  assert.doesNotMatch(countPanel, /position-count-launcher/);
  assert.doesNotMatch(countPanel, /Next uncounted|Finish count|position-count-progress/);
...
test("stock by location opens part detail with its shop and exact storage context", async () => {
  ...
  assert.match(positions, /const occupiedStockPositions = occupiedPositions\(stockPositions\)/);
  assert.match(positions, /const visibleStockPositions = showEmptyPositions \? stockPositions : occupiedStockPositions/);
  assert.match(positions, /const sourceOptions = \[\.\.\.occupiedStockPositions\]/);
  assert.match(positions, /aria-pressed=\{showEmptyPositions\}/);
  assert.match(positions, /showEmptyPositions \? "Hide empty" : `Show empty \(\$\{emptyPositionCount\}\)`/);
  assert.match(positions, /visibleStockPositions\.map/);
  assert.match(positions, /No shelves or bins currently hold this part/);
