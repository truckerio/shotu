...
export function InspectionTemplatesPage({ actor, locations = [] }) {
  ...
  async function load(target = companyId, { preserveDraft = false } = {}) {
    ...
    try {
      ...
      setAssignments((result.assignments || []).map((entry) => ({ ...entry, unitType: entry.applicabilityKey, location: entry.locationId ? "Location" : "All locations" })));
  ...
  return <section className="admin-content">
    ...
    <InspectionTemplatesWorkspace actor={actor} templates={templates} assignments={assignments} selectedTemplateId={selectedId} saveStatus={saveStatus} onChange={change} onPublish={publish} onCreate={create} onCreateRevision={createRevision} onRetrySave={retryLatestSave} onReloadServer={reloadServerVersion} onSelect={setSelectedId} />
  </section>;
}
