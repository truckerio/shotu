...
export function InspectionTemplatesWorkspace({
  templates = [], assignments = [], selectedTemplateId, saveStatus = "saved", onChange, onPublish, onCreate, onCreateRevision, onRetrySave, onReloadServer, onSelect,
}) {
...
function Assignments({ assignments }) {
  return <div className="inspection-template-assignments">{assignments.length ? assignments.map((assignment) => <article key={assignment.id || `${assignment.templateId}-${assignment.location}`}><strong>{assignment.location || "All locations"}</strong><span>{assignment.templateName || "Weekly inspection"}</span><small>{assignment.unitType || "Unit"}</small></article>) : <p>Assignments will appear here after templates are connected to locations.</p>}</div>;
}

function TemplateEditor({ template, saveStatus, compactMode, onCompactMode, onChange, onPublish, onCreateRevision, onRetrySave, onReloadServer }) {
  ...
  return <div className="inspection-template-editor">
    ...
    <footer>{published ? <><p>This version is active.</p><Button onClick={onCreateRevision}>Create revision</Button></> : saveStatus === "error" ? <><p className="inspection-template-errors">Save failed. Your edits are still here.</p><span className="inspection-template-recovery"><Button onClick={onRetrySave}>Retry</Button><Button onClick={onReloadServer}>Reload</Button></span></> : errors.length ? <p className="inspection-template-errors">{errors[0]}</p> : <p>Ready to publish.</p>}{!published ? <Button variant="primary" disabled={Boolean(errors.length) || saveStatus !== "saved"} onClick={onPublish}>Publish</Button> : null}</footer>
  </div>;
}
