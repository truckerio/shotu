...
function InspectionCancellation({ onCancelInspection }) {
  ...
  return <section className="inspection-cancel"><h2>Cancel inspection</h2>{confirming ? <><p>Saved responses remain audit evidence. This inspection will not be completed or added to service history.</p><label>Reason<textarea minLength="2" rows="2" value={reason} onChange={(event) => setReason(event.target.value)} /></label><div className="shared-action-row"><Button type="button" onClick={() => setConfirming(false)}>Keep inspection</Button><Button type="button" disabled={busy || reason.trim().length < 2} onClick={cancel}>{busy ? "Cancelling…" : "Confirm cancellation"}</Button></div></> : <Button type="button" onClick={() => setConfirming(true)}>Cancel inspection</Button>}</section>;
}
...
function InspectionStart({ inspection, onStart, onStarted }) {
  ...
  return <section className="inspection-start" aria-labelledby="inspection-start-title">
    ...
    {inspection.previousReportAvailable ? <label className="inspection-start-review"><Checkbox checked={previousReportReviewed} onChange={(event) => setPreviousReportReviewed(event.target.checked)} /><span>Previous report reviewed</span></label> : null}
