...
export function WorkorderOdooPanel({
  access,
  controller,
  eligible,
  fieldAccess = {},
  ...
  return (
    <div className="surveillance-work-panel workorder-odoo-panel">
      ...
      {!eligible ? (
        ...
        <div className="surveillance-odoo-form" aria-label="Odoo workorder module">
          ...
          <section className="surveillance-odoo-readiness" aria-label="Odoo readiness">
            <div><span>Status</span><strong>{readinessStatus}</strong></div>
            <div><span>Customer</span><strong>{identityLabel(odooReadiness?.customer)}</strong></div>
            <div><span>Vehicle</span><strong>{identityLabel(odooReadiness?.vehicle)}</strong></div>
            <div><span>Warehouse</span><strong>{identityLabel(odooReadiness?.warehouse)}</strong></div>
          </section>
          ...
          ) : <p className="surveillance-progress-state">Read-only Odoo access</p>}
        </div>
      )}
    </div>
  );
