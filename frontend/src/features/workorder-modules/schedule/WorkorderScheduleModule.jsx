...
export function WorkorderScheduleModule({ access, activeSection, allowedActions = {}, endDate, locale, onChange, onSelect, onStartDateChange, presentation = "panel", startDate, workorder }) {
  ...
  return (
    <ProgressiveWorkorderSection id="schedule" className={onePage ? "create-schedule-one-page" : ""} title={t("schedule.title")} activeSection={activeSection} onSelect={onSelect} displayMode="panel" showTitle={!onePage}>
      {canWrite && onePage ? <div className="create-schedule-one-page-fields"><span className="create-schedule-one-page-label">{t("schedule.workDates")}</span><div className="create-schedule-one-page-range"><label><span className="operational-sr-only">{t("schedule.startDate")}</span><DatePicker value={startDate || ""} onChange={(event) => onStartDateChange?.(event.target.value)} aria-label={t("schedule.startDate")} /></label><span className="create-schedule-one-page-separator" aria-hidden="true">→</span><label><span className="operational-sr-only">{t("schedule.endDate")}</span><DatePicker value={endDate || ""} min={startDate || undefined} onChange={(event) => onChange?.("workEndDate", event.target.value)} aria-label={t("schedule.endDate")} /></label></div></div> : canWrite ? (
        <div className="two-col">
          <Field label={t("schedule.startDate")}><DatePicker value={startDate || ""} onChange={(event) => onStartDateChange?.(event.target.value)} aria-label={t("schedule.startDate")} /></Field>
          <Field label={t("schedule.endDate")}><DatePicker value={endDate || ""} min={startDate || undefined} onChange={(event) => onChange?.("workEndDate", event.target.value)} aria-label={t("schedule.endDate")} /></Field>
        </div>
