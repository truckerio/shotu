...
export function receiptLineFacts(draft, suggestion) {
  return (draft?.lines || []).map((line, invoiceLineIndex) => {
    ...
    return {
      ...
      invoiceQuantity: number(match?.invoiceOutstandingQuantity ?? serverFact?.invoiceOutstandingQuantity ?? line?.quantity?.value),
...
export function receiveAllReceiptLines(lines) {
  return (lines || []).map((line) => ({
    ...line,
    acceptedQuantity: String(receiptLineLimit(line)),
    heldQuantity: "",
    rejectedQuantity: "",
    notReceivedQuantity: "",
    outcome: "available",
...
export function receiptLinesReady(lines) {
  if ((lines || []).some((line) => !line.trackingMode)) return false;
