...
export function ReceiptLinesEditor({ runId = "", runVersion = 0, receiptEpisode = 0, draft, suggestion, facts = null, initialLines = null, mode = "receipt", sourceLabel = "Invoice", positions = emptyPositions, positionLoading = false, positionError = "", targetLocked = false, fixedTargetPositionId = "", fixedTargetLabel = "", disabled = false, onChange }) {
  const prefix = useId();
  ...
  return <section className="receipt-lines-editor" aria-labelledby={`${prefix}-title`}>
    ...
    {lines.map((line, index) => {
      ...
      const expanded = expandedLineIndex === index;
      ...
      return <fieldset key={line.invoiceLineIndex} className={`receipt-line-card${expanded ? " is-expanded" : ""}`}>
