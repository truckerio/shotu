...
test("phone queue toolbar keeps shared tabs and filter on one line", async () => {
  ...
  assert.match(phoneRules, /\.workspace-operations \.mobile-queue-toolbar\.mobile-queue-toolbar \.mechanic-queue-tabs\s*\{[\s\S]*grid-template-columns: none;[\s\S]*justify-content: space-between;/);
