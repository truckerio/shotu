...
test("detail and completion surfaces use the shared Work done action", () => {
  assert.equal((detailPage.match(/<WorkDoneButton/g) || []).length, 3);
  assert.match(detailPage, /isOfficeDetail && canMarkWorkDone \? <WorkDoneButton[^>]*onClick=\{\(\) => openOfficeTool\("completion"\)\}/);
