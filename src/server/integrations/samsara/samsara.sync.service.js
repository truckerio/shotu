...
async function fetchSamsaraFleet(client) {
  ...
  for (let index = 0; index < vehicles.length; index += 50) {
    ...
    try {
      ...
      for (const stat of statsBody.data || []) statsByVehicleId.set(String(stat.id), stat);
    } catch (error) {
      if (isRejectedSamsaraApiCredential(error)) throw error;
  ...
  for (let index = 0; index < trailers.length; index += 50) {
    ...
    try {
      ...
      for (const stat of statsBody.data || []) statsByTrailerId.set(String(stat.id), stat);
    } catch (error) {
      if (isRejectedSamsaraApiCredential(error)) throw error;
...
async function runSamsaraSync({
  ...
  let auth = { source: "oauth" };
  try {
    const authenticated = await withSamsaraClient(
      (client) => fetchSamsaraFleet(client),
      { allowApiTokenFallback, companyId },
    );
    auth = authenticated.auth;
    const { statsByTrailerId, statsByVehicleId, trailers, vehicles } = authenticated.result;
