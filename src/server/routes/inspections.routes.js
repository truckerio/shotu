import { z } from "zod";
import { assignInspectionSchema, cancelInspectionSchema, completeInspectionSchema, createInspectionPrintArchiveSchema, createInspectionRevisionSchema, createInspectionSchema, createInspectionWorkorderSchema, inspectionListSchema, inspectionVersionActionSchema, linkInspectionWorkorderSchema, saveInspectionResponsesSchema } from "../modules/inspections/inspection.schemas.js";
import { assignInspection, cancelInspection, completeInspection, createInspectionFindingWorkorder, createInspectionPrintArchiveRecord, eligibleInspectionWorkorders, inspectionCreateContext, linkInspectionToWorkorder, patchInspectionResponses, printInspectionSlip, queryInspectionSummaries, readInspection, readInspectionPrintArchiveRecord, requestInspection, reviseInspection, startInspection } from "../modules/inspections/inspection.service.js";
...
function nestedRoute(pathname) {
  const match = /^\/api\/inspections\/([^/]+)(?:\/assignments|\/workorders|\/revisions|\/findings\/([^/]+)\/workorder-links|\/print-archives(?:\/([^/]+))?)$/.exec(pathname);
  ...
  if (pathname.endsWith("/revisions")) return { id:decodeURIComponent(match[1]), kind:"revisions" };
  if (match[2]) return { id:decodeURIComponent(match[1]), findingId:decodeURIComponent(match[2]), kind:"workorder-links" };
  return { id:decodeURIComponent(match[1]), archiveId:match[3] ? decodeURIComponent(match[3]) : null, kind:"print-archives" };
}
...
export async function handleInspectionsApi(req,res,url,helpers,dependencies={}){
  ...
  if (nested) {
    ...
    if (nested.kind === "revisions" && req.method === "POST") { sendJson(res,201,{inspection:await (dependencies.revise || reviseInspection)(requestContext,nested.id,parse(createInspectionRevisionSchema,await readBody(req)))}); return true; }
    ...
    if (nested.kind === "print-archives" && !nested.archiveId && req.method === "POST") { sendJson(res,201,await (dependencies.createArchive || createInspectionPrintArchiveRecord)(requestContext,nested.id,parse(createInspectionPrintArchiveSchema,await readBody(req)))); return true; }
...
export const inspectionRouteInternals={detailRoute,nestedRoute,parse};
