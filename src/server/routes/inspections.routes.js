...
function detailRoute(pathname){const match=/^\/api\/inspections\/([^/]+)(?:\/(responses|print)|\/actions\/(start|complete|cancel|assign|claim))?$/.exec(pathname);return match?{id:decodeURIComponent(match[1]),responses:match[2]==="responses",print:match[2]==="print",action:match[3]||null}:null;}
...
export async function handleInspectionsApi(req,res,url,helpers,dependencies={}){
  ...
  if(req.method==="POST"&&route.action){const schemas={start:inspectionVersionActionSchema,complete:completeInspectionSchema,cancel:cancelInspectionSchema,assign:assignInspectionSchema,claim:claimInspectionSchema};const actions={start:dependencies.start||startInspection,complete:dependencies.complete||completeInspection,cancel:dependencies.cancel||cancelInspection,assign:dependencies.assign||assignInspection,claim:dependencies.claim||claimInspection};sendJson(res,200,{inspection:await actions[route.action](requestContext,route.id,parse(schemas[route.action],await readBody(req)))});return true;}
  return false;
}
