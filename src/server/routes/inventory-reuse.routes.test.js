...
test("remove, atomic return, legacy receive and review cross real schemas into guarded mutation service",async()=>{
  ...
  assert.equal((await request(`/api/inventory-reuse/${caseId}/return`,"POST",{...scope,exactUnitId:randomUUID(),outcome:"hold",evidence:"Exact unit received",expectedVersion:1,idempotencyKey:"return-test-1"},deps)).status,200);
  ...
  assert.deepEqual(calls.map((c)=>c.action),["remove","return","receive","release"]);
});
test("asset, custody case, operation, config and explicit configuration actions map exact endpoint contracts",async()=>{
  ...
  for(const endpoint of ["",`/asset/${randomUUID()}`,`/cases/${caseId}`,"/operations/original-key","/config"]) assert.equal((await request(`/api/inventory-reuse${endpoint}${search}`,"GET",null,deps)).status,200);
  assert.deepEqual(views,["queue","asset","case","operation","config"]);
...
test("exact custody case route rejects malformed ids and preserves scoped not-found errors",async()=>{
  const search=`?companyId=${companyId}&locationId=${locationId}`;
  assert.equal((await request(`/api/inventory-reuse/cases/not-a-case${search}`,"GET")).status,400);
  const missing=await request(`/api/inventory-reuse/cases/${caseId}${search}`,"GET",null,{read:async()=>{throw new InventoryError("Custody case not found.",{code:"INVENTORY_REUSE_NOT_FOUND",statusCode:404});}});
  assert.equal(missing.status,404);
  assert.equal(missing.data.code,"INVENTORY_REUSE_NOT_FOUND");
});
