...
const input = { expectedVersion: 2, description: "Air valve", partNumber: "A-1", manufacturer: "Bendix", category: "Air", barcode: "123", referenceNumbers: [" BW-1 "] };
...
test("part edit maps stale, identity, provider and hidden tenant failures", async () => {
  for (const [kind, code, status] of [["stale", "INVENTORY_PART_STALE", 409], ["identity_conflict", "INVENTORY_PART_IDENTITY_CONFLICT", 409], ["provider_managed", "INVENTORY_PART_FIELD_PROVIDER_MANAGED", 422], ["not_found", "inventory_not_found", 404]]) {
