...
export const reuseReadSchema = z.object({ ...scope, limit: z.coerce.number().int().min(1).max(100).default(50), cursor: reuseId.optional(), q: z.string().trim().max(200).default(""), status: z.string().trim().max(80).optional(), route: route.optional(), condition: condition.optional(), catalogPartId: reuseId.optional(), code: z.string().trim().max(4000).optional() }).strict();
