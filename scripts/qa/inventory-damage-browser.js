...
await page.route('**/api/office/inventory/stock-tasks?*',r=>r.fulfill({json:{items:[{id:'task',part_number:'PART-101',quantity:2,uom_code:'ea',status:'inspection',holder:'Courier',destination_name:'Workshop',kind:'damage',units:[],events:[]},{id:'po-damage',part_number:'PO-DAMAGE',kind:'damage',delivery_report:true,status:'awaiting_inventory',holder:'Supplier',reason:'Two filters dented',location_name:'Shop',quantity:null,uom_code:'',units:[],events:[]}],hasMore:false}}));
...
for(const name of ['Stock','Review','Damaged stock','Stock and usage','Open purchasing commitments','In transit','Supplier money outstanding']){
...
await page.getByRole('button',{name:'PO-DAMAGE',exact:true}).click();await page.getByText('Two filters dented',{exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'Approve inspected stock for use',exact:true}).count(),0);await page.keyboard.press('Escape');await page.getByRole('button',{name:'PART-101',exact:true}).click();await page.getByRole('dialog').waitFor();assert.deepEqual(errors,[]);console.log(`Inventory tables and actions passed at ${width}px`);await page.close();}}
