...
export function renderInspectionSlip(model) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeInspectionHtml(model.inspectionNumber || "Inspection")}</title><style>@page{size:letter;margin:.45in}*{box-sizing:border-box}body{font:12px Arial,sans-serif;color:#111;margin:0}h1{font-size:20px;margin:0 0 4px}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:4px 18px;border:1px solid #111;padding:8px;margin:10px 0}h2{font-size:13px;margin:10px 0 0;padding:5px 7px;background:#eee;border:1px solid #111}.check{display:grid;grid-template-columns:1fr auto;gap:3px 12px;padding:5px 7px;border:1px solid #bbb;border-top:0;break-inside:avoid}.check small{grid-column:1/-1}.boxes{white-space:nowrap}.notes,.signature{margin-top:12px;border:1px solid #111;padding:8px;min-height:42px}.signature{display:grid;grid-template-columns:1fr 160px;gap:24px;margin-top:18px}.watermark{position:fixed;transform:rotate(-30deg);font-size:70px;color:#ddd;top:40%;left:18%}</style></head><body>${watermark}<h1>${escapeInspectionHtml(model.templateLabel || "Weekly Inspection")}</h1><div class="meta"><span><strong>Inspection:</strong> ${escapeInspectionHtml(model.inspectionNumber)}</span><span><strong>Unit:</strong> ${escapeInspectionHtml(model.unit?.unitNo)}</span><span><strong>VIN:</strong> ${escapeInspectionHtml(model.unit?.vin)}</span><span><strong>Plate:</strong> ${escapeInspectionHtml(model.unit?.licensePlate)}</span><span><strong>Result:</strong> ${escapeInspectionHtml(model.result || "")}</span><span><strong>Completed:</strong> ${escapeInspectionHtml(model.completedAt ? new Date(model.completedAt).toLocaleString("en-US") : "")}</span></div>${sections}<div class="notes"><strong>Notes</strong><p>${escapeInspectionHtml(model.finalNotes || "")}</p></div><div class="signature"><span>Mechanic signature: ______________________________</span><span>Date: ______________</span></div></body></html>`;
}
